...
export function AdminDashboardPage() {
  ...
  // Dummy timeline for now since we don't have historical days in DB seeded over a week yet
  const RATINGS_TIMELINE_DATA = [
    { name: 'Mon', count: Math.floor(allRatings.length * 0.1) },
    { name: 'Tue', count: Math.floor(allRatings.length * 0.15) },
    { name: 'Wed', count: Math.floor(allRatings.length * 0.1) },
    { name: 'Thu', count: Math.floor(allRatings.length * 0.2) },
    { name: 'Fri', count: Math.floor(allRatings.length * 0.25) },
    { name: 'Sat', count: Math.floor(allRatings.length * 0.15) },
    { name: 'Sun', count: Math.floor(allRatings.length * 0.05) },
  ];
  ...
  return (
    <div className="min-h-screen bg-background flex flex-col selection:bg-pastel-lavender">
        ...
        {activeTab === 'overview' && (
          ...
          >
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-5">
              {/* KPI 1: Users */}
              <div className="p-6 rounded-3xl bg-pastel-lavender/40 border border-purple-100 shadow-soft">
                <div className="flex justify-between items-start mb-4">
                  <div className="w-11 h-11 rounded-2xl bg-white shadow-2xs flex items-center justify-center text-primary">
                    <Users className="w-5 h-5" />
                  </div>
                  <span className="text-xs font-bold text-purple-900 bg-white/80 px-2.5 py-1 rounded-full border border-brand/20/60">
                    {adminKPIs.usersTrend}
                  </span>
                </div>
                <p className="text-xs font-heading font-semibold text-muted mb-1">Total Registered Users</p>
                <h3 className="text-3xl font-heading font-bold text-primary">{adminKPIs.totalUsers.toLocaleString()}</h3>
              </div>

              {/* KPI 2: Stores */}
              <div className="p-6 rounded-3xl bg-pastel-mint/40 border border-emerald-100 shadow-soft">
                <div className="flex justify-between items-start mb-4">
                  <div className="w-11 h-11 rounded-2xl bg-white shadow-2xs flex items-center justify-center text-primary">
                    <Building2 className="w-5 h-5" />
                  </div>
                  <span className="text-xs font-bold text-emerald-900 bg-white/80 px-2.5 py-1 rounded-full border border-emerald-200/60">
                    {adminKPIs.storesTrend}
                  </span>
                </div>
                <p className="text-xs font-heading font-semibold text-muted mb-1">Total Stores Registered</p>
                <h3 className="text-3xl font-heading font-bold text-primary">{adminKPIs.totalStores.toLocaleString()}</h3>
              </div>

              {/* KPI 3: Ratings */}
              <div className="p-6 rounded-3xl bg-pastel-peach/40 border border-amber-100 shadow-soft">
                <div className="flex justify-between items-start mb-4">
                  <div className="w-11 h-11 rounded-2xl bg-white shadow-2xs flex items-center justify-center text-primary">
                    <Star className="w-5 h-5" />
                  </div>
                  <span className="text-xs font-bold text-amber-900 bg-white/80 px-2.5 py-1 rounded-full border border-amber-200/60">
                    {adminKPIs.ratingsTrend}
                  </span>
                </div>
                <p className="text-xs font-heading font-semibold text-muted mb-1">Total Ratings Submitted</p>
                <h3 className="text-3xl font-heading font-bold text-primary">{adminKPIs.totalRatings.toLocaleString()}</h3>
              </div>

              {/* KPI 4: Average */}
              <div className="p-6 rounded-3xl bg-pastel-yellow/40 border border-yellow-200/60 shadow-soft">
                <div className="flex justify-between items-start mb-4">
                  <div className="w-11 h-11 rounded-2xl bg-white shadow-2xs flex items-center justify-center text-primary">
                    <TrendingUp className="w-5 h-5" />
                  </div>
                  <span className="text-xs font-bold text-yellow-900 bg-white/80 px-2.5 py-1 rounded-full border border-yellow-300/60 flex items-center gap-1">
                    {adminKPIs.averageRating} <Star className="w-3 h-3 fill-yellow-900" />
                  </span>
                </div>
                <p className="text-xs font-heading font-semibold text-muted mb-1">Platform Average Rating</p>
                <h3 className="text-3xl font-heading font-bold text-primary flex items-end gap-1">
                  {adminKPIs.averageRating}
                  <Star className="w-6 h-6 fill-pastel-yellow text-pastel-yellow mb-1" />
                </h3>
              </div>
            </div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
              ...
              <div className="lg:col-span-8 p-7 rounded-3xl bg-white border border-borderSoft shadow-soft">
                <div className="flex justify-between items-center mb-6">
                  ...
                  <span className="text-xs font-semibold px-3 py-1 rounded-full bg-pastel-lavender/60 text-purple-950">
                    Last 7 Days
                  </span>
                </div>
            ...
            <div className="bg-white p-4 rounded-2xl border border-borderSoft shadow-2xs flex flex-col sm:flex-row gap-3">
              <div className="flex-1 relative">
                ...
                <input
                  ...
                  className="w-full pl-10 pr-4 py-2.5 rounded-xl bg-gray-50 border border-borderSoft text-xs focus:outline-none focus:ring-2 focus:ring-pastel-mint"
                />
              </div>
              ...
              <div className="flex items-center gap-2">
                ...
                <select
                  ...
                  className="px-3 py-2 rounded-xl bg-gray-50 border border-borderSoft text-xs font-semibold focus:outline-none focus:ring-2 focus:ring-pastel-mint cursor-pointer"
                >
                      ...
                      <tr key={store.id} className="hover:bg-gray-50/60 transition-colors">
                        <td className="py-4 px-6">
                          <div className="flex items-center gap-3">
                            <div className="w-9 h-9 rounded-xl bg-pastel-lavender/60 flex items-center justify-center font-heading font-bold text-sm text-primary">
                              {store.name.charAt(0)}
                            </div>
                            <div>
                        ...
                        <td className="py-4 px-6">
                          <div className="inline-flex items-center gap-1 bg-pastel-yellow/60 px-2.5 py-1 rounded-lg border border-amber-200/50">
            ...
            <div className="bg-white p-4 rounded-2xl border border-borderSoft shadow-2xs flex flex-col md:flex-row justify-between gap-3">
              <div className="flex-1 relative">
                ...
                <input
                  ...
                  className="w-full pl-10 pr-4 py-2.5 rounded-xl bg-gray-50 border border-borderSoft text-xs focus:outline-none focus:ring-2 focus:ring-pastel-lavender"
                />
              </div>
                  ...
                  <tbody className="divide-y divide-gray-100 text-sm">
                    {filteredUsers.map((user) => (
                      ...
                      >
                        <td className="py-4 px-6">
                          <div className="flex items-center gap-3">
                            <div className="w-9 h-9 rounded-xl bg-pastel-peach/60 flex items-center justify-center font-heading font-bold text-sm text-primary">
                              {user.name.charAt(0)}
                            </div>
                          ...
                          <span
                            className={`text-xs px-2.5 py-1 rounded-full font-semibold ${
                              user.role === 'SYSTEM_ADMIN'
                                ...
                                : 'bg-pastel-lavender text-purple-900'
                            }`}
                          >
                            {user.role}
                          </span>
