...
exports.deleteUser = exports.createUser = exports.getUsers = exports.getDashboardStats = void 0;
...
const getDashboardStats = async (req, res) => {
    try {
        ...
        const avgAggregate = await db_1.default.rating.aggregate({
            _avg: { score: true },
        });
...
const getUsers = async (req, res) => {
    try {
        ...
        formattedUsers.sort((a, b) => {
            ...
            if (sortField === 'email_asc')
                return a.email.localeCompare(b.email);
            if (sortField === 'email_desc')
                return b.email.localeCompare(a.email);
            if (sortField === 'role_asc')
                return a.role.localeCompare(b.role);
            if (sortField === 'role_desc')
                return b.role.localeCompare(a.role);
            if (sortField === 'address_asc')
                return (a.address || '').localeCompare(b.address || '');
            if (sortField === 'address_desc')
...
const deleteUser = async (req, res) => {
    try {
        ...
        await db_1.default.rating.deleteMany({ where: { userId: id } });
        await db_1.default.store.deleteMany({ where: { ownerId: id } });
